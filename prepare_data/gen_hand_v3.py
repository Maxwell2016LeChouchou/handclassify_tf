...
def gen_hand_minibatch_thread(size, start_idx, annotation_lines, hand_save_dir, base_nums, class_num):
...
def gen_hand_minibatch(size, start_idx, annotation_lines, hand_save_dir, base_nums, thread_num, class_num):
    num_images = len(annotation_lines)
    ...
    for t in range(thread_num):
        ...
        cur_thread = MyThread(gen_hand_minibatch_thread,(size, start_idx+cur_start_idx, cur_annotation_lines,
                                                        hand_save_dir, base_nums, class_num))
...
def gen_hand_for_one_image(size, idx, ori_img, save_dir, annotation, base_nums, class_num):
    ...
    while hand_num < base_num:
        ...
        # cur_size = int(npr.randint(110, 166)*0.01*bbox_size) # ----------------基于144 修改这里的边界 
...
def gen_hand(anno_file, size=20, base_num = 1, thread_num = 4, class_num = 10):
    save_dir = '/data4/wjz/gesture/hengda-wutong/data_aug_v2/%d'%(size)
    ...
    f = open(os.path.join(save_dir, 'hand_10_09.txt'), 'w')    
    ...
    while start_idx < num:
        ...
        hand_names, cur_hand_nums = gen_hand_minibatch(size, start_idx, cur_annotation_lines, \
                                            hand_save_dir, base_nums, thread_num, class_num)
...
def parse_args():
    ...
    parser.add_argument('--class_num', dest='class_num', help='class num', default='12', type=str)
...
if __name__ == '__main__':
    ...
    print(args)
    gen_hand(args.anno_file, int(args.size), int(args.base_num), int(args.thread_num), int(args.class_num))
